...
def model_predict(model, inputs):
    # repeat greyscale 3 times
    return model([inputs.repeat(1,3,1,1,1)])
...
def get_torch_dataset(csv_path, split):
    ...
    return FramesDensesampleDataset(csv_path, mode,
            ...
            crop_size=crop_size,
            mean = model_cfg.input_mean[:1], std = model_cfg.input_std[:1],
            normalise = model_cfg.input_normalise, greyscale=True,
