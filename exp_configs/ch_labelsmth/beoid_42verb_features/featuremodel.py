...
def get_features(split):
    ...
    if feature_input_type == 'RGB':
        feature_pickle_path = dataset_cfg.RGB_features_pickle_path[split_num][split]
        ...
        video_ids, labels, features = d['video_ids'], d['labels'], d['clip_features'][0]
        if features.shape[1] != input_feature_dim:
            # features are not averaged. Average now
            features = np.mean(features, axis=1)

    elif feature_input_type == 'flow':
        feature_pickle_path = dataset_cfg.flow_features_pickle_path[split_num][split]
        logger.info(f'Generating train pseudo labels using {feature_input_type} features at: {feature_pickle_path}')
        ...
        RGB_feature_pickle_path = dataset_cfg.RGB_features_pickle_path[split_num][split]
        flow_feature_pickle_path = dataset_cfg.flow_features_pickle_path[split_num][split]
        ...
        RGB_video_ids, RGB_labels, RGB_features = d['video_ids'], d['labels'], d['clip_features'][0]
        ...
        flow_video_ids, flow_labels, flow_features = d['video_ids'], d['labels'], d['clip_features'][0]
...
def generate_train_pseudo_labels():
    ...
    if loss_type in loss_types_pseudo_generation:
        if rank == 0:
            assert not use_ideal_train_labels, 'use_ideal_train_labels not implemented.'
            if not use_ideal_train_labels:
                ...
                # ideal labels
                assert dataset_cfg.__name__ == 'dataset_configs.ch_beta.confusing_hmdb_102_features', f'Wrong dataset {dataset_cfg.__name__} for the option use_ideal_train_labels.'
                ...
                multilabels = []
                for singlelabel in feature_data['labels']:
                    another_label = find_another_label(singlelabel)
                    multilabel = np.zeros(dataset_cfg.num_classes, dtype=int)
                    multilabel[singlelabel] = 1
                    multilabel[another_label] = 1
                    multilabels.append(multilabel)
...
best_metric = Clip_mAPMetric(activation='sigmoid')
...
telegram_reporter = DefaultTelegramReporter(include_exp_rootdir = True, ignore_figures = False)
