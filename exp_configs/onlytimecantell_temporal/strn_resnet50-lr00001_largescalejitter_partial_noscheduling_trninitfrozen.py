...
from ...utils.loader import model_load_state_dict_partial

input_frame_length = 8


def optimiser(params):
    return torch.optim.SGD(params, lr = 0.00001, momentum = 0.9, weight_decay = 5e-4)
...
def scheduler(optimiser, iters_per_epoch, last_epoch=-1):
    ...
    return multistep_lr_nonlinear_iters(optimiser, iters_per_epoch, [3], [10], last_epoch=last_epoch)
    #return None
...
def load_model():
    ...
    checkpoint = torch.load('data/experiments/onlytimecantell/trn_resnet50/lr01_largescalejitter_partial_steplr/weights/epoch_0057.pth', map_location = "cuda:{}".format(cur_device))
    model_load_state_dict_partial(model, checkpoint["model_state"])

    for param in model.base_model.model.parameters():  # freeze resnet backbone
