...
input_frame_length_grey = input_frame_length * 3
...
def model_predict(model, inputs):
    ...
    assert C == 1
    # should be N, TC, H, W but C ==1 so doesn't matter
    return model(inputs.view(N, -1, H, W))
...
def get_torch_dataset(csv_path, split):
    ...
    return FramesSparsesampleDatasetAdvanced(csv_path, mode, input_frame_length_grey, 
