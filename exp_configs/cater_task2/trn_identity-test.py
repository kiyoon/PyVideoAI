...
from ...dataloaders.cater_task2 import CaterTask2Dataset 

import torch


input_frame_length = 64
...
def model_predict(model, inputs):
    return model(inputs.view(inputs.shape[0], inputs.shape[1], -1))
...
def get_torch_dataset(csv_path, mode):
    return CaterTask2Dataset(csv_path, os.path.join(dataset_cfg.dataset_root, 'scenes'), mode, input_frame_length)

def _get_torch_dataset_split(split):

    mode = dataset_cfg.split2mode[split]
    csv_path = os.path.join(dataset_cfg.dataset_root, 'lists/actions_order_uniq', dataset_cfg.split_file_basename[split])
