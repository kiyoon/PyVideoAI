...
def train_iter(model, optimiser, scheduler, criterion, data, data_unpack_func, video_metrics, start_time=None, it=None, total_iters=None, sample_seen=None, num_correct_preds=None, total_samples=None, loss_accum=None, rank = 0, world_size = 1, input_reshape_func = None):
    ...
    if rank == 0:
        ...
        # clip mAP
        video_metrics.add_clip_predictions(uids, outputs, labels, apply_activation='sigmoid')

        elapsed_time = time.time() - start_time
#        eta = int((total_samples-sample_seen) * elapsed_time / sample_seen)
        eta = int((total_iters-(it+1)) * elapsed_time / (it+1))
        sys.stdout.write("\r Train Iter: {:4d}/{:4d} - Sample: {:6d}/{:6d} - ETA: {:4d}s - lr: {:.8f} - batch_loss: {:.4f} - loss: {:.4f}        ".format(it+1, total_iters, sample_seen, total_samples, eta, lr, batch_loss, loss))
        sys.stdout.flush()
    else:
        loss = None
        elapsed_time = None

    return sample_seen, num_correct_preds, loss_accum, loss, elapsed_time, lr#}}}
...
def train_epoch(model, optimiser, scheduler, criterion, dataloader, data_unpack_func, rank = 0, world_size = 1, input_reshape_func = None):
    ...
    Returns:
        tuple: sample_seen, total_samples, loss, vid_mAP, elapsed_time, video_metrics
    """
    ...
    for it, data in enumerate(dataloader):
        ...
        sample_seen, num_correct_preds, loss_accum, loss, elapsed_time, lr = train_iter(model, optimiser, scheduler, criterion, data, data_unpack_func, video_metrics, start_time, it, total_iters, sample_seen, num_correct_preds, total_samples, loss_accum, rank, world_size, input_reshape_func)

    if rank == 0:
        vid_mAP = video_metrics.mAP()

        sys.stdout.write("\r")
        sys.stdout.flush()
        logger.info(" Train Iter: {:4d}/{:4d} - Sample: {:6d}/{:6d} - {:d}s - lr: {:.8f} - loss: {:.4f} - vid_mAP: {:.4f}                                                           ".format(it+1, total_iters, sample_seen, total_samples, round(elapsed_time), lr, loss, vid_mAP))
    else:
        vid_mAP = None

    return sample_seen, total_samples, loss, vid_mAP, elapsed_time, video_metrics#}}}
...
def eval_epoch(model, criterion, dataloader, data_unpack_func, num_classes, batch_size, one_clip = False, rank = 0, world_size = 1, input_reshape_func = None, scheduler=None, PAD_VALUE = -1):
    ...
    Returns:
        tuple: sample_seen, total_samples, loss, vid_mAP, elapsed_time, video_metrics (VideoMetrics), eval_log_str
    """
...
#                labels = torch.IntTensor([]).to(cur_device, non_blocking=True)
            ...
            if rank == 0:
                ...
                if one_clip:
                    ...
                    eval_mode = "Multi-clip Eval"
                sys.stdout.write("\r {:s} Iter: {:4d}/{:4d} - Sample: {:6d}/{:6d} - ETA: {:4d}s - val_loss: {:.4f}        ".format(eval_mode, it+1, total_iters, sample_seen, total_samples, eta, loss))
                sys.stdout.flush()
...
#        dataloader.reset()
    ...
    if rank == 0:
        ...
        eval_log_str = " {:s} Iter: {:4d}/{:4d} - Sample: {:6d}/{:6d} - {:d}s - val_loss: {:.4f} - val_vid_mAP: {:.4f}".format(eval_mode, it+1, total_iters, sample_seen, total_samples, round(elapsed_time), loss, vid_mAP)
    ...
    return sample_seen, total_samples, loss, vid_mAP, elapsed_time, video_metrics, eval_log_str#}}}
