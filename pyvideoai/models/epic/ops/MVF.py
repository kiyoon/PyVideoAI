...
from .se_module import HardSwish
import logging
...
def make_multi_view_fusion(
    ...
    if len(list(net.layer3.children())) >= 23:  # R101 & R152
        # n_round = 2
        logger.info('=> Using n_round {} to insert MVF module'.
                format(n_round))

    def make_block_MVF(stage, this_segment):
        """build MVF Block"""
        blocks = list(stage.children())
        logger.info('=> Processing stage with {} {} blocks residual'.format(len(blocks), mode))
...
class MVF(nn.Module):
    ...
    def __init__(self, net, n_segment, in_channels, alpha=0.5, use_hs=True, share=False, mode='THW'):
        ...
        logger.info('=> Using Multi-view Fusion...')
