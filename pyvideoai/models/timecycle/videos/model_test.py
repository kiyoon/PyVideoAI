# Notice of change by Kiyoon Kim
# Removed unused parameters `class_num` and `temporal_out`
# Removed param pretrained because it is a test model.
...
class CycleTime(nn.Module):
    ...
    def __init__(self, dim_in=2048, trans_param_num=6, detach_network=False, T=None):
        super(CycleTime, self).__init__()

        dim = 512

        resnet = resnet_res4s1.resnet50(pretrained=False)
