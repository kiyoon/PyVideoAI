...
def pyav_decode_sparse(
    container, num_output_frames, uniform=True
):
    """
    Convert the video from its original fps to the target_fps. If the video
    support selective decoding (contain decoding information in the video head),
    ...
    # Frame indices to filter later 
    absolute_frame_indices = utils.sparse_frame_indices(frames_length, num_output_frames, uniform)

    if duration is None:
        # If failed to fetch the decoding information, decode the entire video.
        decode_all_video = True
        video_start_pts, video_end_pts = 0, math.inf
        relative_frame_indices = absolute_frame_indices
    else:
        # Perform selective decoding.
        decode_all_video = False
        start_idx, end_idx = absolute_frame_indices[0], absolute_frame_indices[-1]
        relative_frame_indices = [idx - start_idx for idx in absolute_frame_indices]      # shift the frame indices by start_idx
        timebase = duration / frames_length
        video_start_pts = int(start_idx * timebase)
        video_end_pts = int(end_idx * timebase)
    ...
    if container.streams.video:
        ...
        print(relative_frame_indices)
        frames = [video_frames[i] for i in relative_frame_indices]       # filter frames
        frames = [frame.to_rgb().to_ndarray() for frame in frames]
        frames = torch.as_tensor(np.stack(frames))
    return frames, fps, absolute_frame_indices
